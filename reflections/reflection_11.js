...
//can use it to convert a logical composition to be a conditional expression
...
function transform_logical_composition_to_conditional_expression(comp) {
    const symbol = logical_symbol(comp);
    const lhs = logical_composition_first_component(comp);
    const rhs = logical_composition_second_component(comp);
    if (symbol === "&&") {
        return make_conditional_expression(lhs, rhs, make_literal(false));
    } else {
        return make_conditional_expression(lhs, make_literal(true), rhs);
    }
}
...
function evaluate(component, env) { 
    return is_literal(component)
           ...
           ? evaluate(transform_logical_composition_to_conditional_expression(component), env)
...
//Extend_environment is used to update the environment with the new locals and
//unassigned variables/constants
...
//Pair the new frame that contains a list of symbols and values with the base_env
//env will be a list of frames where the end is a null
//Time we call extend_environment: eval_block
...
// setting up global environment that can handle functions
/* MODIFIED for Q1 */
...
//Global environment is basically a list or pair that contains 1 frame
/*Frames that contain primitive_function_symbols, primitive_constant_symbols etc.
*/
